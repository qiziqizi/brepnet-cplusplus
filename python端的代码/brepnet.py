...
def build_matrix_Psi(Xf, Xe, Kf, Ke):
    """
    ...
    are in the rows of the integer tensors Kf, Ke.

    We use these indices to extract the appropriate rows of
    the feature matrices Xf, Xe.  These are then 
    ...
    # appropriate rows of Xf, Xe into 2 tensors
    ...
    Psi = torch.cat([Pt, Pe], dim=1)

    return Psi


def build_matrix_Psi_local(Xf, Xe, Kf, Ke):
    """
    Build the matrix Psi.

    The input consists of the feature tensors

    Xf.size() = [ num_coedges x (num_ents * num_ent_features) ]
    Xe.size() = [ num_edges x num_edge_features ]
...
class BRepNetLayer(LightningModule):
    ...
    def __init__(self, num_mlp_layers, input_size, output_size, isFirstLayer, use_LCS, dropout=None):
        """
        ...
        # 这个变量涉及当前BRepNetLayer中的mlp的relu激活函数的使用
        ...
        self.mlp = BRepNetMLP(num_mlp_layers, input_size, 2*output_size, 2*output_size, final_layer, dropout)

        self.isFirstLayer = isFirstLayer
        self.use_LCS = use_LCS
        

    def forward(self, Xf, Xe, Kf, Ke, Ce, Cf, Csf):
        """
        ...
        if self.isFirstLayer and self.use_LCS:
            Psi = build_matrix_Psi_local(Xf, Xe, Kf, Ke)
        else:
            Psi = build_matrix_Psi(Xf, Xe, Kf, Ke)

        # Next the mlp is applied to Psi
        Z = self.mlp(Psi)

        # Now we need to split Z into 2 parts
        Ze = Z[:, : self.output_size]
        Zf = Z[:, self.output_size :]

        He = find_max_feature_vectors_for_each_edge(Ze, Ce)
        Hf = find_max_feature_vectors_for_each_face(Zf, Cf, Csf, self.device)

        return (Hf, He, Kf, Ke, Ce, Cf, Csf)
...
class BRepNet(LightningModule):
    ...
    def __init__(self, opts):
        ...
        if opts.use_face_grids:
            if opts.no_uvs:
                inchannels_surface = 7
            else:
                inchannels_surface = 9
            self.surface_encoder = UVNetSurfaceEncoder(in_channels=inchannels_surface, output_dims=surf_embedding_size)
        if opts.use_edge_grids or opts.use_coedge_grids:
            if opts.no_uvs:
                inchannels_curve = 12
            else:
                inchannels_curve = 13
            self.curve_encoder = UVNetCurveEncoder(in_channels=inchannels_curve, output_dims=curve_embedding_size)
        ...
        mlp_hidden_size = num_filters*(num_faces_per_kernel + num_edges_per_kernel)
        ...
        self.layers.append(BRepNetLayer(num_mlp_layers, mlp_input_size, num_filters, True, self.opts.use_local_grids_only, dropout))

        # The hidden layers has a size based on in the number of filters
        for l in range(2, opts.num_layers):
            self.layers.append(BRepNetLayer(num_mlp_layers, mlp_hidden_size, num_filters, False, self.opts.use_local_grids_only, dropout))
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--test_res_dir",  type=str, help="")
        ...
        parser.add_argument("--num_classes", type=int, default=27, help="Number of classes used in the dataset")
        ...
        parser.add_argument("--use_local_grids_only", action="store_true", help="only use local coordinate grids, no feature")
        parser.add_argument("--no_uvs", action="store_true", help="do not use UV in the grids")
        parser.add_argument('--gpuid', type=str, default=0, help="")
        return parser
    ...
    def create_face_embeddings_local(self, Gf_local, Ge_local, Kf, Ke, Ce, Cf, Csf):
        num_u = 10
        num_v = 10
        if self.opts.no_uvs:
            face_grid_channel = 7
            ...
            face_grid_channel = 9
        Gf_local_reshaped = Gf_local.view(Gf_local.shape[0] * 2, face_grid_channel, num_u, num_v)    # 每个coedge关联到两个面的grid（f与mf）

        # 这里的Xf实际上已经处理成build_matrix_psi函数中Pt所对应内容
        # 即每个coedge关联到的两个face对应的编码结果合并到同一行
        Xf = self.surface_encoder(Gf_local_reshaped).view(Gf_local.shape[0], self.opts.surf_embedding_size * 2)    # [num_coedges x (num_ents * num_ent_features)]
        Xe = self.curve_encoder(Ge_local)

        for i, layer in enumerate(self.layers):
            Xf, Xe, Kf, Ke, Ce, Cf, Csf = layer(Xf, Xe, Kf, Ke, Ce, Cf, Csf)

        return self.output_layer(Xf, Xe, Kf, Ke, Ce, Cf, Csf)


    def create_face_embeddings(self, Gf, Ge, Kf, Ke, Ce, Cf, Csf):
        """
    ...
    def forward(self, Gf, Ge, Kf, Ke, Ce, Cf, Csf, Gf_local, Ge_local):
        """
        A forward pass through the network.
        """
        if self.opts.use_local_grids_only:
            assert self.opts.use_face_grids == 1 and self.opts.use_edge_grids == 1
            face_embeddings = self.create_face_embeddings_local(Gf_local, Ge_local, Kf, Ke, Ce, Cf, Csf)
        else:
            face_embeddings = self.create_face_embeddings(Gf, Ge, Kf, Ke, Ce, Cf, Csf)
    ...
    def brepnet_step(self, batch, batch_idx, save_segmentation_output):
        ...
        if self.opts.no_uvs:
            Gf = batch["face_point_grids"][:, :7, :, :]
            Ge = batch["edge_point_grids"][:, :12, :]
            Gf_local = batch["face_point_grids_local"][:, :, :7, :, :]
            Ge_local = batch["edge_point_grids_local"][:, :12, :]
        else:
            Gf = batch["face_point_grids"]
            Ge = batch["edge_point_grids"]
            Gf_local = batch["face_point_grids_local"]
            Ge_local = batch["edge_point_grids_local"]
        ...
        if self.opts.test_res_dir is not None:
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.Adam(self.parameters(), lr=self.opts.learning_rate)
